 ...
 * @version 1.0
 */
public class FIFOOrdering implements Ordering {
    private static final Logger logger = LoggerFactory.getLogger(FIFOOrdering.class);
    ...
    private VectorClock<UUID> vc  = new VectorClock<UUID>(ManagementModule.PID);
    ...
    public Message prepareOutgoingMessage(Message m, GroupView g) {
        ...
        VectorClock<UUID> vc = new VectorClock<UUID>(ManagementModule.PID,
                                                   msgCounter.get());
        m.setVectorClock(vc);
            ...
            if (otherHasSent == (hasReceived + 1)) {
                logger.debug("otherHasSent: {}, hasReceived: {}, from: {}",
                             new Object[] {otherHasSent,
                                           hasReceived,
                                           m.getOriginUID().equals(ManagementModule.PID) ? "ME" : m.getOriginUID().toString()});
